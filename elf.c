...
int load_segment(int fd, Elf64_Addr p_vaddr, Elf64_Addr p_filesz,
                 Elf64_Off p_offset) {
  int err;
  size_t cur_offset;
  ...
  regions[region_num] = mmap((void *)p_vaddr - offset, mmap_length[region_num],
...
void setup_stack_and_jump(Elf64_Addr e_entry) {
  void *stack =
      mmap((void *)(0x700000000000), 1024 * 1024 * 1024, PROT_WRITE | PROT_READ,
           MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
*/
  __asm__("mov $0xdeadbeefabcd1234,%rdx;" // dl_random, 16 bytes of random
          "push %rdx;"
          "push %rdx;");
  __asm__("push $0;"   // aux
          "push $0;"   // aux
...
// Load elf binary from file into new address region of current process, then
// run it
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < header.e_shnum; i++) {
    ...
    if (!strncmp(str_table + sh_header.sh_name, ".bss", 4)) {
      ...
      regions[region_num] =
          mmap((void *)((sh_header.sh_addr / 4096 + 1) * 4096),
               mmap_length[region_num], PROT_WRITE | PROT_READ,
