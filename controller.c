...
static int load_segment(int fd, Elf64_Addr p_vaddr, Elf64_Addr p_filesz,
                        Elf64_Off p_offset,
  ...
  if (ibv_post_send(helper_context->qp, &send_wr, &send_bad_wr))
    DEBUG_PRINT(
        "oh god, post_send didn't work when sending region request..\n");
  ...
  while (1) {
    ...
    if (wc.wr_id == RECV_OPID) {
      ...
      if (rpc->type == rpc_region_response) {
        ...
        mmapped_region = mmap(0, mmap_length, PROT_WRITE | PROT_READ,
                              MAP_PRIVATE, fd, p_offset - offset);
        if (mmapped_region == (void *)-1)
          DEBUG_PRINT("shit, mmap failed on controller, errno: %d\n", errno);

        // Now register this as a new memory region
        new_mr = ibv_reg_mr(helper_context->pd, mmapped_region, mmap_length,
        ...
        if (ibv_post_send(helper_context->qp, &send_wr, &send_bad_wr))
          DEBUG_PRINT(
              "oh god, post_send didn't work.. while trying one-sided RDMA\n");
        else
...
static int send_binary(struct ibv_helper_context *helper_context,
                       struct ibv_recv_wr *recv_wr) {
  struct ibv_wc wc;
  struct ibv_recv_wr *bad_wr;
  struct ibv_send_wr *send_bad_wr;
