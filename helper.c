...
int create_helper_context(struct ibv_context *ctx, struct ibv_helper_context *helper_context) {
    ...
    ERR(ibv_query_qp(helper_context->qp, &query_qp_attr, IBV_QP_QKEY | IBV_QP_STATE | IBV_QP_PATH_MTU | IBV_QP_PORT , &query_init_attr));
...
int init_qp(struct ibv_helper_context *helper_context, int remote_qp_num) {
     ...
     */
    ERR(ibv_modify_qp(helper_context->qp, &qp_attr, IBV_QP_STATE | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS | IBV_QP_PKEY_INDEX));
    ...
    ERR(ibv_modify_qp(helper_context->qp, &qp_attr, IBV_QP_STATE | IBV_QP_AV | IBV_QP_PATH_MTU |
                IBV_QP_DEST_QPN | IBV_QP_RQ_PSN));

    //We also need to move qp to a RTS state....
    qp_attr.qp_state = IBV_QPS_RTS;
    qp_attr.sq_psn = helper_context->qp->qp_num;
    ERR(ibv_modify_qp(helper_context->qp, &qp_attr, IBV_QP_STATE | IBV_QP_SQ_PSN));
